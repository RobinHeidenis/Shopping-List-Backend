...
Item.create = (newItem: item, result) => {
    query("SELECT count(*) FROM robindb.shopping_list")
        .then((res) =>
            query("INSERT INTO robindb.shopping_list (name, quantity, url, sequence) VALUES (?, ?, ?, ?)", [newItem.name, newItem.quantity, newItem.url, res[0][Object.keys(res[0])[0]]]))
        .then((res) =>
            query("SELECT * FROM robindb.shopping_list WHERE id = ?", [res.insertId])
                .then((res) => {
